...
// ── Collection definitions (metadata only — no products here) ──
...
// ── Fetch ALL collections from Supabase in one request ──
async function fetchAllCollectionProducts() {
  ...
  // Convert array to map: { "gifts-for-boyfriend": [...products], ... }
  const map = {};
  rows.forEach((row) => {
    map[row.collection] = row.products;
  });
  return map;
}
    ...
    <div className="grid grid-cols-3 h-[130px] gap-px bg-[#F6F3F0]">
      {[0, 1, 2].map((i) => (
        <div key={i} className="w-full h-full"
          style={{
            background: "linear-gradient(90deg,#EDE8E3 0%,#F6F3F0 50%,#EDE8E3 100%)",
...
const Collection = () => {
  ...
  // Map of cacheKey → products array from Supabase
  const [productsMap,   setProductsMap]   = useState({});
  const [loadingImages, setLoadingImages] = useState(true);

  // ✅ Fetch all collection products from Supabase once on mount
  // This is ONE database read — not an API call to SerpAPI
  useEffect(() => {
    fetchAllCollectionProducts()
      .then((map) => setProductsMap(map))
      .catch((err) => console.warn("Could not load collection images:", err))
      .finally(() => setLoadingImages(false));
  }, []);

  // Double collections for seamless infinite scroll
  ...
  return (
    <section className="py-20" id="feed">
      ...
      <div className="relative overflow-hidden mt-12">
        ...
        <div className="flex gap-5 w-max animate-feedScroll hover:pause-feedScroll py-3 px-2">
          {SCROLL_COLLECTIONS.map((c, i) => {
            ...
            // ✅ Get products for this collection from Supabase data
            const products   = productsMap[c.cacheKey] || [];
            // ✅ Use first 3 product images for the card grid
            ...
            return (
              ...
              >
                {/* ✅ 3 real product images from Supabase — or skeleton while loading */}
      ...
      {/* Modal — passes full products array from Supabase */}
      {activeModal !== null && (
        <CollectionModal
          collection={{
            ...COLLECTIONS_META[activeModal],
            products: productsMap[COLLECTIONS_META[activeModal].cacheKey] || [],
          }}
